...
Output: Maximum Temperature in a Year For Each Weather Station
'''
...
# Removes any non TMAX records
maxTemps = parsedLines.filter(lambda x: "TMAX" in x[1])
# We don't need the ObservationType column anymore since its all TMAX, so remove it
stationTemps = maxTemps.map(lambda x: (x[0], x[2]))
# Get the highest temp for each station
